...
- The owning template's manifest exists but is not a regular file — a FIFO, a socket, a device, a directory, or a dangling symlink stands at its manifest path: the CLI refuses with `CONTENT_CONFLICT`, naming the unreadable path, rather than computing `toDelete`/`toPreserve` from an exclusion set silently emptied by the read failure it could not honestly answer — a manifest genuinely ABSENT (nothing yet exists there, or the local origin folder no longer resolves inside the project root) is a different fact and is not refused: its `excludedSubtrees` is treated as empty, since there is no declaration on record to disagree with an empty one (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-if-manifest-absent`/`inst-dp-if-manifest-unreadable`).
...
**Output**: `{ toDelete, toPreserve }` — the plan's delete and preserve lists — or `TARGET_NOT_APPLIED` when `<target>` matches no template's applied `targets` array, or `CONTENT_CONFLICT` when the owning template's manifest cannot be read.
...
3. [x] - `p1` - Read the owning template's declared `excludedSubtrees` and compute `<target>`'s effective ownership by the same six-term subtraction the apply path computes (`inst-ua-compute-ownership`, CLI-5): `<target>` minus `excludedSubtrees` minus `projectOwnedRoots` beneath it minus `.frontx` minus the reserved environment entries (`.git`, `.DS_Store`, `Thumbs.db`) beneath it minus the template's own local origin folder (when installed by local path). The local-origin term is not optional here: a `path:`-installed template whose origin folder sits beneath its own target would otherwise have that folder — the developer's own source for the template — computed into this target's owned ground and removed - `inst-dp-compute-ownership`
   1. [x] - `p1` - **IF** the owning template's manifest is genuinely ABSENT — the local inventory holds no content for a remote-origin name, or, for a local `path:` origin, nothing exists yet at its manifest path (or its origin folder can no longer be proven to stay inside the project root, which reading a manifest from would meet the identical absence one step later regardless) - `inst-dp-if-manifest-absent`
      1. [x] - `p1` - Treat its `excludedSubtrees` as empty rather than refusing: there is no exclusion declaration on record anywhere for this step to disagree with an empty one, this is the ordinary shape of a name whose local origin folder a developer has since removed by hand, and every OTHER applied instance nested under `<target>` stays protected independently regardless (`inst-dp-find-nested`) - `inst-dp-else-manifest-absent-empty`
   2. [x] - `p1` - **ELSE IF** the owning template's manifest exists but cannot be read as a regular file — a FIFO, a socket, a device, a directory, or a dangling symlink stands where it is expected - `inst-dp-if-manifest-unreadable`
