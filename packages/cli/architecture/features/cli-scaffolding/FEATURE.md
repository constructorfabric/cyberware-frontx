...
- The owning template's declared `excludedSubtrees` cannot be established from EITHER source this flow now consults, in this order: the CURRENT manifest — genuinely ABSENT, unreadable as a regular file (a FIFO, a socket, a device, a directory, or a dangling symlink), its origin folder no longer resolvable inside the project root, or read but no longer validating against the four-field contract — AND no value RECORDED on the project state entry either (a legacy entry from before that field existed, or one whose current manifest and recorded value are BOTH unusable): the CLI refuses with `CONTENT_CONFLICT`, naming the template and the remedy (re-register the origin), rather than computing `toDelete`/`toPreserve` from an exclusion set silently emptied by a fact it could not honestly answer — an origin folder removed from disk is the ORDINARY lifecycle of a vendored `path:` origin, not an exotic failure, and treating any of these as `[]` previously let a developer's own `excludedSubtrees`-protected file land in `toDelete` under `ok:true` (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-else-refuse-unestablished`). Whenever the CURRENT manifest CAN be read and validates, it wins over any recorded value — the ONE resolution order `cpt-frontx-feature-composed-provenance`'s own `cpt-frontx-dod-composed-provenance-contract-ownership` fixes, so `delete` and `assemble`/`apply` never disagree about a name's declared exclusions.
...
11. [x] - `p1` - **RETURN** success — the deleted and preserved lists, the owning template name, whether this deletion emptied it, and `aiBundleResidue` when the CLI-owned bundle removal above failed (absent, never `null`, otherwise) - `inst-del-return-success`
...
**Output**: `{ toDelete, toPreserve }` — the plan's delete and preserve lists — or `TARGET_NOT_APPLIED` when `<target>` matches no template's applied `targets` array, or `CONTENT_CONFLICT` when the owning template's manifest cannot be read, or when it carries no recorded `excludedSubtrees` and its declaration cannot otherwise be established from its current manifest.
...
3. [x] - `p1` - Compute `<target>`'s effective ownership by the same six-term subtraction the apply path computes (`inst-ua-compute-ownership`, CLI-5): `<target>` minus the owning template's declared `excludedSubtrees` (established by the sub-steps below) minus `projectOwnedRoots` beneath it minus `.frontx` minus the reserved environment entries (`.git`, `.DS_Store`, `Thumbs.db`) beneath it minus the template's own local origin folder (when installed by local path). The local-origin term is not optional here: a `path:`-installed template whose origin folder sits beneath its own target would otherwise have that folder — the developer's own source for the template — computed into this target's owned ground and removed - `inst-dp-compute-ownership`
   1. [x] - `p1` - Resolve the owning template's CURRENT manifest through the one shared formulation every other consumer of this fact uses. Any reason that source cannot supply a declaration — the manifest is genuinely ABSENT, it cannot be read as a regular file (a FIFO, a socket, a device, a directory, or a dangling symlink stands where it is expected), or its origin folder can no longer be proven to stay inside the project root — is caught here rather than allowed to propagate: none of those honestly says "there is no declaration at all", only that THIS source could not supply one - `inst-dp-resolve-current-manifest`
   2. [x] - `p1` - **IF** that manifest was read AND still validates against the four-field contract — use its declared `excludedSubtrees`. The CURRENT manifest wins over any recorded value so this algorithm and the apply path never disagree about a name's declared exclusions while the origin is readable: a developer who edited the vendored manifest after registration previously had `delete` honour the stale recorded list while `assemble` read the edited one, and a file the current manifest declared protected was deleted on the strength of that disagreement - `inst-dp-if-current-manifest-valid`
   3. [x] - `p1` - **ELSE IF** the owning template's project-state entry carries a RECORDED `excludedSubtrees` (`cpt-frontx-contract-project-provenance`'s `TemplateEntry.excludedSubtrees`, written when the name is registered and re-recorded from the NEW payload's own declaration at its most recent upgrade) — use that. Deletion must not depend on a `path:` origin folder still being on disk: vendoring one into the project and removing it again is the ORDINARY lifecycle, and the recorded value is the last declaration this store actually verified - `inst-dp-else-if-recorded-exclusions`
   4. [x] - `p1` - **ELSE** (neither source established a declaration) **RETURN** `CONTENT_CONFLICT` naming the owning template and saying its declared `excludedSubtrees` could not be established, with the remedy — re-register the origin (`frontx register <origin> --replace`), which records the declaration onto an entry missing it even when the `{origin, version}` pair is otherwise unchanged; nothing deleted, and `<target>` stays in its template's `targets[]`. NEVER folded to `[]`: a live repro disproved that shape, an origin folder removed by hand putting a developer's own `excludedSubtrees`-protected file into `toDelete` under `ok:true`, and a plan computed from an exclusion set silently emptied by an inconclusive answer states a blast radius nobody verified - `inst-dp-else-refuse-unestablished`
...
9. [x] - `p1` - **RETURN** `{ toDelete, toPreserve }` - `inst-dp-return-plan`
...
- [x] `delete <target>` refuses with `CONTENT_CONFLICT`, naming the unreadable path, when the owning template's manifest exists but is not a regular file (a FIFO, a socket, a device, a directory, or a dangling symlink) — never computing `toDelete`/`toPreserve` from an exclusion set the read failure silently emptied, and never placing a path the manifest would have declared excluded onto `toDelete`. A genuinely ABSENT manifest is not refused the same way: its `excludedSubtrees` is treated as empty, since no declaration is on record to disagree with an empty one.
