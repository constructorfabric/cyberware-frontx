...
3. [x] - `p1` - **FOR EACH** path in the payload set - `inst-ec-foreach-payload-path`
   1. [x] - `p1` - **IF** the payload path itself is reported as a symlink, or any directory component of it — from the project root down to the path's own parent, the target's own components included — is reported as a symlink or as an ordinary regular file where a directory is required. The range reaches above the target because a regular file occupying a component at or above it blocks the payload path just as one below it does, and canonicalizing the target proves only that it resolves inside the project root, never that every component of it is a directory - `inst-ec-if-symlink-component`
      1. [x] - `p1` - Add it to `contentConflicts` and move on to the next payload path, without checking whether anything exists at the path by name. A symlink cannot be compared against the payload's declared text content, and writing through one lands somewhere the payload path does not name; a regular file standing where an ancestor directory component belongs makes the payload path unreachable on disk the way the payload declares it — either way "cannot be compared at all and refuses the same way, fail-closed, with `CONTENT_CONFLICT`" (`architecture/ADR/0021-project-upgrade-mechanism.md`, the identical ground already settled for the upgrade engine, whose own `inst-cls-if-not-regular` makes the identical extension to a regular-file ancestor). This is a conflict in BOTH modes: `--adopt-existing` means leaving what is already there untouched, and a write that follows a symlink, or that attempts to create a directory where a file already stands, touches something else entirely — never the thing adoption was asked to leave alone. The refusal names the specific offending component — the payload path itself, or the ancestor that blocked it — so a developer is told, for example, that `"app/dir" is a regular file` rather than being left to walk the path themselves - `inst-ec-add-symlink-conflict`
...
- [x] For a target not yet recorded, existing on-disk content matching the payload exactly is reported as `identicalFiles` and causes no refusal; content differing from the payload at a payload path is reported as `contentConflicts` and refuses the whole batch, writing no file; content at a path the payload does not declare is reported as `additionalPaths` and refuses the batch unless `--adopt-existing` is given, in which case it is left untouched. A payload path standing on, or beneath, a symlink, or beneath a regular file occupying a directory's position, is always `contentConflicts` — never `identicalFiles`, even when the link's target happens to hold byte-identical content, and never adopted, since `--adopt-existing` means leaving what is already there untouched and a write through a symlink, or into a directory a file already occupies, touches something else entirely; the refusal names the specific offending component.
