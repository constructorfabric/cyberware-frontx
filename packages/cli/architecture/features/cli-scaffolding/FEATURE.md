...
- The owning template's CURRENT manifest and its RECORDED declaration both establish an `excludedSubtrees` value but the two disagree (a vendored manifest edited, or a registration re-recorded, after the other side last changed): the computed plan still unions both — nothing either source protects is ever un-protected by the disagreement — and additionally reports the drift (both values) in every outcome the plan reaches (`--dry-run`, `CONFIRMATION_REQUIRED`, declined, and a completed deletion), never only in a code comment, so a developer confirming the deletion can see that the two sources disagree and which entries came from which.
...
- `<target>` IS found among a registered template's `targets` array, but its own on-disk location is no longer an ordinary directory — replaced by a regular file, or any other non-directory entry, since it was applied: the CLI refuses with `CONTENT_CONFLICT`, naming the target, rather than enumerating an empty candidate set (which would read as "nothing here to preserve or delete", false) or letting a bare filesystem error reach the caller with no structured envelope. This check runs on every plan computation — the initial one and the recomputation immediately before a confirmed deletion — so a target replaced between the two computations is still caught.
- The owning template's declared `excludedSubtrees` cannot be established from EITHER source this flow consults — the CURRENT manifest (genuinely ABSENT, unreadable as a regular file, its origin folder no longer resolvable inside the project root, or read but no longer validating against the four-field contract) AND no value RECORDED on the project state entry either (a legacy entry from before that field existed, or one whose current manifest and recorded value are BOTH unusable): the CLI refuses with `CONTENT_CONFLICT`, naming the template, rather than computing `toDelete`/`toPreserve` from an exclusion set silently emptied by a fact it could not honestly answer — an origin folder removed from disk is the ORDINARY lifecycle of a vendored `path:` origin, not an exotic failure, and treating any of these as `[]` previously let a developer's own `excludedSubtrees`-protected file land in `toDelete` under `ok:true` (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-else-refuse-unestablished`). The remedy named depends on which failure this is: an origin confirmed genuinely absent names `unregister <name>` (nothing is left to re-register), while a merely unreadable one names `register <origin> --replace`. Whenever BOTH sources CAN be established, this algorithm resolves the declared exclusions as their UNION, never one overriding the other — `cpt-frontx-feature-composed-provenance`'s own `cpt-frontx-dod-composed-provenance-contract-ownership` still fixes the CURRENT manifest as the SOLE source `assemble`/`apply`/`ownership` resolve, a deliberately DIFFERENT question from what a deletion may safely remove, and this algorithm's plan reports an `exclusionsDrift` fact whenever the two sources it unions actually disagree, so a developer confirming a deletion can see that they do.
...
2. [x] - `p1` - Resolve WHICH recorded target the caller means before resolving WHERE on disk to act: compare the caller's own spelling, lexically normalized, against every registered template's `targets[]` first, and only when nothing matches compare the canonicalized form against the canonicalized form of each recorded entry. Canonicalization answers where the ground is, never which record the caller means — a target applied under one name whose on-disk location has since become a symlink canonicalizes to a path `targets[]` never held, and keying the lookup off that would leave the recorded target unreachable through the only command that can remove it. Everything downstream — the plan, the confirmation report, and the `targets[]` entry the deletion removes — uses the RECORDED spelling this step returns - `inst-del-resolve-recorded-name`
3. [x] - `p1` - The CLI invokes the deletion-plan algorithm (`cpt-frontx-algo-cli-scaffolding-delete-plan`) against `<target>` - `inst-del-compute-plan`
4. [x] - `p1` - **IF** `<target>` is not found in any template's `targets` array, or the deletion-plan algorithm otherwise refuses — `CONTENT_CONFLICT` when the owning template's declared `excludedSubtrees` could not be established from either the current manifest or a recorded value (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-else-refuse-unestablished`) — this flow does not re-derive either refusal a second way, it surfaces the algorithm's own result unchanged - `inst-del-if-not-applied`
   1. [x] - `p1` - **RETURN** the algorithm's own refusal — `TARGET_NOT_APPLIED` naming the target as not an applied instance of any registered template, or the algorithm's own `CONTENT_CONFLICT` naming the template and the remedy - `inst-del-return-not-applied`
   2. [x] - `p1` - **ELSE IF** `<target>` IS found, but its own on-disk location, or any component of the path leading to it, is no longer an ordinary directory — replaced by a regular file, or any other non-directory entry, since `<target>` was applied. The component that blocks the path is named whether it is `<target>` itself or one above it: a blocked ancestor makes the target exactly as unenumerable, and is otherwise indistinguishable from a target whose ground a developer removed by hand, which this flow deliberately treats as an ordinary empty deletion. The identical fact `apply` and `upgrade` already refuse for a payload path whose disk shape cannot be compared, applied here to the TARGET's own shape rather than a path beneath it: its recorded content can no longer be enumerated, compared, or removed the way a directory's content is. This check runs on every plan computation this flow makes — the initial one here and the recomputation immediately before a confirmed deletion, whichever route (`--json --yes` or interactive) reaches it — so a target replaced between the two computations is still caught - `inst-del-if-target-shape-drifted`
      1. [x] - `p1` - **RETURN** `CONTENT_CONFLICT`, naming the blocking component; nothing deleted, and `<target>` stays in its template's `targets[]` — striking a target from the record without having been able to look at its ground would report a deletion that never happened - `inst-del-return-target-shape-drifted`
5. [x] - `p1` - **IF** `--dry-run` was given - `inst-del-if-dry-run`
   1. [x] - `p1` - **RETURN** the delete/preserve lists, plus the plan's `exclusionsDrift` fact when one was computed; nothing is deleted and no confirmation is required - `inst-del-return-dry-run`
6. [x] - `p1` - **IF** `--json` was given - `inst-del-if-json`
   1. [x] - `p1` - **IF** `--yes` was not given - `inst-del-if-json-no-yes`
      1. [x] - `p1` - **RETURN** `CONFIRMATION_REQUIRED` with the delete/preserve lists, plus the plan's `exclusionsDrift` fact when one was computed; no prompt is shown and stdin is never read; nothing deleted - `inst-del-return-confirmation-required`
...
7. [x] - `p1` - **ELSE** (interactive) - `inst-del-else-interactive`
   1. [x] - `p1` - Prompt the developer to confirm, defaulting to No — showing the plan's `exclusionsDrift` fact alongside the delete/preserve lists when one was computed, so an interactive developer sees the same disagreement a `--json` caller reads from the envelope - `inst-del-prompt`
   2. [x] - `p1` - **IF** the developer declines - `inst-del-if-declined`
      1. [x] - `p1` - **RETURN** nothing deleted, carrying the plan's `exclusionsDrift` fact when one was computed - `inst-del-return-declined`
8. [x] - `p1` - Remove the deletion plan's ground from disk, preserving every path in the plan's preserve list - `inst-del-remove`
9. [x] - `p1` - Remove `<target>` from its template's `targets` array in the project state store (`cpt-frontx-feature-composed-provenance`) - `inst-del-update-state`
10. [x] - `p1` - **IF** this removal empties the owning template name's `targets` array - `inst-del-if-last-target`
   1. [x] - `p1` - The CLI removes that name's CLI-owned AI-extension bundle at `.frontx/ai/<manifest-name>/`, if present (`cpt-frontx-algo-cli-scaffolding-ai-bundle`). By this point the target's ground is already off disk and its template-state entry already updated (steps 8–9 above), both real and correct; if this bundle removal itself fails, that failure is not surfaced as a refused operation over a deletion that in fact already happened — it is carried forward as `aiBundleResidue` (manifest name, path, and the failure's own message) on the success outcome step 11 returns, rather than reported as `ok: false` or silently dropped - `inst-del-remove-bundle`
11. [x] - `p1` - **RETURN** success — the deleted and preserved lists, the plan's `exclusionsDrift` fact when one was computed, the owning template name, whether this deletion emptied it, and `aiBundleResidue` when the CLI-owned bundle removal above failed (absent, never `null`, otherwise) - `inst-del-return-success`
...
1. [x] - `p1` - **FOR EACH** target under check (a staged batch target, or a candidate `ownership add` path) - `inst-cc-foreach-target`
   1. [x] - `p1` - Canonicalize the target to a project-relative POSIX path; a symlink or a `..` segment can never resolve outside the project root. The canonical form is what is reported and recorded, not the string the caller typed: a caller naming a symlink is told, and has stored in `targets[]`, the real location its content went to. A DANGLING symlink is resolved the same way — through the text of the broken link — so a target reached that way materializes at the path the link names even though nothing stood there before; that stays inside the project root, and the applied target reported back names it, but it is not the path the caller spelled. For ground that already exists on disk, canonicalization also resolves to its actual ON-DISK spelling, never the caller's own — the supported platforms this constraint binds (`cpt-frontx-constraint-cli-platform-path-identity`, CLI DESIGN §2.2) - `inst-cc-canonicalize`
...
3. [x] - `p1` - **FOR EACH** pair of targets in the combined set in which at least one side is a target under check - `inst-cc-foreach-pair`
   1. [x] - `p1` - **IF** the two targets are identical and claimed by the same template name — "identical" decided per the project volume's own case sensitivity (`cpt-frontx-constraint-cli-platform-path-identity`): byte-exact on a case-sensitive volume, case-insensitive on one that is not, so a batch's own two spellings of one not-yet-created target are recognized as the same ground before either is ever materialized - `inst-cc-if-same-template-noop`
      1. [x] - `p1` - Treat the pair as an idempotent no-op, not a conflict - `inst-cc-noop`
   2. [x] - `p1` - **IF** the two targets are identical (by that same volume-aware rule) and claimed by two different template names - `inst-cc-if-same-target-diff-template`
      1. [x] - `p1` - Record a conflict naming the contested target and both templates - `inst-cc-record-same-target`
   3. [x] - `p1` - **IF** one target is a strict ancestor of the other — decided by whole path segments of each canonicalized target, never by string-prefix comparison, so `packages/app` and `packages/app-shell` share a string prefix but no path segment and are siblings, not ancestor and descendant; segments compare per the same volume-aware identity rule - `inst-cc-if-ancestor`
...
**Output**: Three partitions — `identicalFiles` (already on disk, matching the payload exactly), `contentConflicts` (already on disk at a payload path, differing from it, **or** standing on, or beneath, a symlink; a directory where the payload declares a file; or a special filesystem entry (a FIFO, socket, or device) — either at that payload path or on the way to it — see step 3.1 below, an exception to "matching exactly" that holds even when a symlink's target happens to hold byte-identical content), `additionalPaths` (already on disk within the target's effective ownership area, at a path the payload does not write — an ordinary directory the walk recurses into on the way to a real payload path is never itself reported here, only the file, symlink, or special entries found within it), and `uncomparablePaths` — the subset of `contentConflicts` refused because a symlink, a directory, or a special entry stands at the payload path, or a symlink, a regular file, or a special entry stands on the way to it, rather than because the content there differs, together with `uncomparableCauses` naming, for each such path, the specific component responsible (the payload path itself, or the offending ancestor) and its kind — reported separately so a refusal can name each path's own cause and its own remedy (resolve the link, move the file or directory, or reconcile the edit) instead of offering both and leaving the reader to guess — or, when nothing pre-exists, all partitions empty.
...
3. [x] - `p1` - **FOR EACH** path in the payload set - `inst-ec-foreach-payload-path`
   1. [x] - `p1` - **IF** the payload path itself is reported as a symlink, a directory, or a special filesystem entry (a FIFO, socket, or device) — or any directory component of it — from the project root down to the path's own parent, the target's own components included — is reported as a symlink, a special filesystem entry, or as an ordinary regular file where a directory is required. The range reaches above the target because a regular file occupying a component at or above it blocks the payload path just as one below it does, and canonicalizing the target proves only that it resolves inside the project root, never that every component of it is a directory. A directory occupying the payload path's OWN position is checked only at the leaf, never as an ancestor condition — an ordinary directory ancestor is exactly the required shape for everything beneath it - `inst-ec-if-symlink-component`
      1. [x] - `p1` - Add it to `contentConflicts` and move on to the next payload path, without checking whether anything exists at the path by name. A symlink cannot be compared against the payload's declared text content, and writing through one lands somewhere the payload path does not name; a regular file standing where an ancestor directory component belongs makes the payload path unreachable on disk the way the payload declares it; a directory standing exactly at the payload path itself makes the declared file unreachable there too; and a special filesystem entry (a FIFO, socket, or device) can never be read or written safely at all — attempting to read one blocks indefinitely on a FIFO with no writer attached, which this algorithm's own real reader must never do. Either way "cannot be compared at all and refuses the same way, fail-closed, with `CONTENT_CONFLICT`" (`architecture/ADR/0021-project-upgrade-mechanism.md`, the identical ground already settled for the upgrade engine, whose own `inst-cls-if-not-regular` makes the identical extension to a regular-file ancestor). This is a conflict in BOTH modes: `--adopt-existing` means leaving what is already there untouched, and a write that follows a symlink, that attempts to create a directory or a file where the wrong shape already stands, or that would open a special file at all, touches something else entirely — never the thing adoption was asked to leave alone. The refusal names the specific offending component and its kind — the payload path itself, or the ancestor that blocked it — so a developer is told, for example, that `"app/dir" is a regular file` or `"app/README.md" is a directory` rather than being left to walk the path themselves - `inst-ec-add-symlink-conflict`
...
5. [x] - `p1` - **RETURN** `identicalFiles`, `contentConflicts`, `additionalPaths`, `uncomparablePaths` — the subset of `contentConflicts` step 3.1 recorded, never a fourth disjoint partition, so a caller reading only the first three behaves exactly as before — and `uncomparableCauses`, naming the specific component step 3.1 blamed for each `uncomparablePaths` entry - `inst-ec-return-partitions`
...
**Output**: `{ toDelete, toPreserve }` — the plan's delete and preserve lists, plus an `exclusionsDrift` fact (present only when the owning template's CURRENT manifest and its RECORDED declaration were both established and disagree) — or `TARGET_NOT_APPLIED` when `<target>` matches no template's applied `targets` array, or `CONTENT_CONFLICT` when NEITHER the owning template's current manifest NOR its recorded declaration can establish what it excludes.
...
3. [x] - `p1` - Compute `<target>`'s effective ownership by the same six-term subtraction the apply path computes (`inst-ua-compute-ownership`, CLI-5): `<target>` minus the owning template's declared `excludedSubtrees` (established by the sub-steps below) minus `projectOwnedRoots` beneath it minus `.frontx` minus the reserved environment entries (`.git`, `.DS_Store`, `Thumbs.db`) beneath it minus the template's own local origin folder (when installed by local path). The local-origin term is not optional here: a `path:`-installed template whose origin folder sits beneath its own target would otherwise have that folder — the developer's own source for the template — computed into this target's owned ground and removed - `inst-dp-compute-ownership`
   1. [x] - `p1` - Resolve the owning template's CURRENT manifest through the one shared formulation every other consumer of this fact uses. Any reason that source cannot supply a declaration — the manifest is genuinely ABSENT, it cannot be read as a regular file (a FIFO, a socket, a device, a directory, or a dangling symlink stands where it is expected), or its origin folder can no longer be proven to stay inside the project root — is caught here rather than allowed to propagate: none of those honestly says "there is no declaration at all", only that THIS source could not supply one. A genuinely ABSENT origin (a clean "nothing there" rather than a thrown read failure) is distinguished from a merely UNREADABLE one here too, since only the former means re-registering this origin can never succeed (step 4 below) - `inst-dp-resolve-current-manifest`
   2. [x] - `p1` - **IF** that manifest was read AND still validates against the four-field contract — record its declared `excludedSubtrees` as the CURRENT declaration - `inst-dp-if-current-manifest-valid`
   3. [x] - `p1` - Read the owning template's project-state entry's RECORDED `excludedSubtrees` (`cpt-frontx-contract-project-provenance`'s `TemplateEntry.excludedSubtrees`, written when the name is registered and re-recorded from the NEW payload's own declaration at its most recent upgrade), when present, as the RECORDED declaration — read UNCONDITIONALLY, never gated on whether step 2 supplied a CURRENT declaration, since the union below needs both, never either alone - `inst-dp-read-recorded-declaration`
   4. [x] - `p1` - **IF** NEITHER the CURRENT declaration (step 2) NOR the RECORDED declaration (step 3) was established, **RETURN** `CONTENT_CONFLICT` naming the owning template and saying its declared `excludedSubtrees` could not be established; nothing deleted, and `<target>` stays in its template's `targets[]`. The remedy named is whichever one can actually run: when the origin was confirmed genuinely ABSENT (step 1), re-registering it cannot help — nothing is left to resolve — so the remedy is `unregister <name>` instead, the escape `cpt-frontx-algo-composed-provenance-unregister` grants for exactly this unusable-orphan state; when the origin is merely UNREADABLE (present but not a regular file), the remedy is re-registering it (`frontx register <origin> --replace`), which records the declaration onto an entry missing it even when the `{origin, version}` pair is otherwise unchanged - `inst-dp-else-refuse-unestablished`
   5. [x] - `p1` - **ELSE** set the declared `excludedSubtrees` to the UNION of the RECORDED declaration and the CURRENT declaration (each defaulting to empty when not established), deduplicated. Two earlier resolution orders each mirrored the hazard the other one fixed: "RECORDED always wins" let a stale record widen a plan once its origin vanished, and "CURRENT always wins" let a developer NARROW the vendored manifest after `apply` and un-protect ground the RECORDED declaration protected when the template was applied — reproduced live: register with `excludedSubtrees: ["userland/"]`, apply, narrow the manifest to `[]`, and `delete` swept the developer's own `userland/` file into `toDelete`. The union is safe in both directions: narrowing the manifest can never un-protect ground the RECORDED declaration already protected, and a vanished origin (nothing for the CURRENT source to supply) can never widen the plan beyond what the RECORDED declaration allows. `apply`/`assemble`/`ownership` are NOT changed to match — they materialize what the manifest says TODAY, a different question, and over-preserving in a deletion is the safe direction (a preserved file is named in the list the developer confirms, while a deleted one is gone) - `inst-dp-union-declared-exclusions`
   6. [x] - `p1` - **IF** both the CURRENT declaration and the RECORDED declaration were established AND they name different sets — record that drift (both values) on this algorithm's own returned result, so a developer confirming a deletion computed from the union can see THAT the manifest and the record disagree and WHICH entries came from which; not recorded when either source is unestablished (nothing to compare) or when the two agree - `inst-dp-if-exclusions-drift`
4. [x] - `p1` - Identify every other template's target that is a strict descendant of `<target>` — a nested applied instance belonging to a different template, decided by the same volume-aware ground identity `inst-cc-if-ancestor` uses (`cpt-frontx-constraint-cli-platform-path-identity`), so two templates whose targets were recorded under two spellings of the same case-insensitive ground are recognized as nested rather than as unrelated - `inst-dp-find-nested`
5. [x] - `p1` - Identify every OTHER registered template's local origin folder (installed by local path) that is a strict descendant of `<target>` — a different template's own source, distinct from that template's applied target and never itself declared as an exclusion by the owning template being deleted, since the owning template has no reason to know about a folder that belongs to someone else's registration. This step reads no manifest content at all — the origin folder is derived from the recorded `path:` origin string alone (`deriveLocalOriginFolder`), so the absent-versus-unreadable distinction step 3 above draws does not arise here: there is no read to fail - `inst-dp-find-other-origins`
6. [x] - `p1` - Identify every entry inside `<target>` that cannot be enumerated as a regular file — a FIFO, a socket, a device, a dangling symlink. Such an entry occupies ground inside the target and survives the deletion, because the removal step acts on regular files only; it therefore belongs in `toPreserve`. Leaving it in neither list would break the property the confirmation gate rests on: that the two lists together state the blast radius before it is executed, so a developer approving a deletion is told what goes and what stays - `inst-dp-find-unenumerable`
7. [x] - `p1` - Set `toPreserve` to `excludedSubtrees` beneath `<target>`, every nested target found, every other template's local origin folder found, the OWNING template's own local origin folder when it too lies beneath `<target>`, every `projectOwnedRoots` entry beneath `<target>`, and every reserved environment entry beneath `<target>` — so a target `.` at the project root never lists `.git`, `.DS_Store`, or `Thumbs.db` in `toDelete`, since they were already subtracted from `<target>`'s effective ownership in the prior step. The owning template's own origin folder is named for the same reason a `projectOwnedRoots` entry is: both are the DEVELOPER's own ground, and `cpt-frontx-adr-template-ownership-boundary-declaration` rests delete's safety on these lists stating the blast radius before it is executed — a developer whose template source sits under the target would otherwise watch it survive with nothing in the report saying why. `.frontx` stays out on the opposite ground: it is CLI-owned, not the developer's, and no confirmation decision turns on it - `inst-dp-set-preserve`
8. [x] - `p1` - Set `toDelete` to `<target>`'s effective ownership minus every path in `toPreserve`. `toDelete` names files, never the directories that hold them: a directory the removal leaves empty is left standing, and pruning it is deliberately not part of this plan. A confirmed deletion removes exactly what the list it was confirmed against named, which is the property delete's whole confirmation gate rests on; pruning would remove ground the list never mentioned. It is also the identical choice `cpt-frontx-feature-upgrade-changeset` already fixes for the mirror case — an upgrade unlinks each `REMOVE` operation's path "without removing the directory it leaves empty" - `inst-dp-set-delete`
9. [x] - `p1` - **RETURN** `{ toDelete, toPreserve }`, plus the `exclusionsDrift` fact from step 3.6 when one was recorded - `inst-dp-return-plan`
...
1. [x] - `p1` - **IF** this operation just gave `name` its first target - `inst-aib-if-first-target`
   1. [x] - `p1` - **IF** the name's installed content path contains a `.frontx/ai/<manifest-name>/` folder - `inst-aib-if-bundle-present`
      1. [x] - `p1` - **IF** a component strictly between `.frontx/ai/` and the bundle's own destination, once followed all the way through when it is a symlink, exists as something other than a directory — a regular file, a FIFO, a socket, a device, or a dangling symlink — remove that entry before it is ever needed as a directory: `.frontx/ai/` is ground the CLI alone writes and removes (`cpt-frontx-adr-template-ownership-boundary-declaration`), so nothing else could have legitimately placed content there, and the entry removed is always the blocking component's OWN position, never whatever a symlink there resolves to. A DIRECTORY found at that same component, OR A SYMLINK RESOLVING TO ONE, is never removed by this step: a directory chain is built through either exactly the same way, so it is traversed rather than reclaimed — it legitimately holds another scoped name's own bundle, or aliases a developer's own directory the bundle is written straight into - `inst-aib-reclaim-ancestor-blocker`
...
4. [x] - `p1` - The existence check both branches above perform (`inst-aib-if-bundle-present`, `inst-aib-if-bundle-exists`) answers **false** — not present — when a non-directory ancestor blocks a scope component strictly above the bundle path being probed, exactly as it already answers false for genuine absence: no valid bundle can stand beneath a non-directory, whatever the reason a scope component looks that way today. This is what lets `upgrade`'s own bundle refresh — which composes `LAST_TARGET_LOST` then `FIRST_TARGET_GAINED` against this SAME algorithm, through the SAME seams, to realize one "refresh" transition neither branch names on its own — reach the `FIRST_TARGET_GAINED` half's own reclaim step (`inst-aib-reclaim-ancestor-blocker`) instead of failing this existence probe outright, matching the ordinary, actionable outcome `apply` already reaches for the identical shape rather than an internal error - `inst-aib-exists-blocked-ancestor`
5. [x] - `p1` - **IF** a scope component of `.frontx/ai/` — or the bundle's own destination itself — resolves, once every symlink along it is followed, to somewhere OUTSIDE the REAL `.frontx/ai/` directory (`cpt-frontx-adr-template-ownership-boundary-declaration`, which makes that whole namespace ground the CLI alone writes and removes): CLI-owned ground is defined by RESOLVED LOCATION, not by spelling, so an alias to a developer's own directory elsewhere in the project is never this namespace's own ground merely because its lexical path starts with `.frontx/ai/` - `inst-aib-alias-guard`
   1. [x] - `p1` - **RETURN**/refuse naming both the alias and where it actually resolves to; nothing is cleared, removed, or written at the aliased location — this refusal runs BEFORE the copy transition's own ancestor-reclaim step and BEFORE either transition's own destination-clearing/removal step, so an aliasing scope component is never reached by either. A symlink that resolves to somewhere STILL inside the real `.frontx/ai/` directory is traversed exactly as before this check existed - `inst-aib-alias-guard-fail`
...
The system **MUST** treat a target already recorded under its named template's `targets[]` entry in the project state store as an idempotent no-op by that record alone — reading no on-disk content and running no existing-content reconciliation for it, regardless of whether the on-disk content still matches what was last applied. For every target a batch was cleared to write to and **not** already so recorded, the system **MUST**, before materializing it, reconcile the template's payload against whatever already exists on disk within that target's effective ownership area, reporting `identicalFiles`, `contentConflicts`, and `additionalPaths` separately: the system **MUST** refuse the whole batch, writing no file, when any unrecorded target reports a `contentConflicts` entry or reports `additionalPaths` without `--adopt-existing`, and **MUST NOT** silently overwrite differing content. A payload path standing on, or beneath, a symlink is never a no-op even when the link's target happens to hold byte-identical content, a payload path beneath a regular file occupying a directory's position — at, above, or below the target — is never a no-op either, a payload path where a DIRECTORY occupies exactly the position the payload declares a file is never a no-op, and a payload path standing on, or beneath, a special filesystem entry (a FIFO, socket, or device) is never a no-op — the system **MUST** report each of these as `contentConflicts`, in both apply modes, and **MUST NOT** attempt to open a special filesystem entry for reading at all, since doing so can block indefinitely with no writer attached: a symlink cannot be compared against declared text content and a write that follows one lands somewhere the payload path does not name, a regular file where a directory is required makes the path unreachable on disk the way the payload declares it, a directory where a file is required makes the declared file unreachable there, and a special entry can never be read or written safely regardless of position — either way naming the specific offending component and its kind in the refusal (`cpt-frontx-algo-cli-scaffolding-existing-content`'s own `inst-ec-if-symlink-component`). An intentional overwrite of already-recorded, already-applied content is available only through `upgrade` (`cpt-frontx-feature-upgrade-changeset`); a repeated `apply` never re-inspects a recorded target's on-disk content and so can never move it onto new content (`target`).
...
The system **MUST** compute a target's deletion plan as that target's effective ownership — the six-term subtraction CLI-5 fixes, applied whole and never as a subset, so the plan is bounded by exactly the ground `apply` computed as owned — minus any nested target belonging to another template and minus any other registered template's local origin folder found beneath the target, through the Conflict Checker's canonicalized geometry, and **MUST** execute it only under explicit confirmation defaulting to no, or, in `--json` mode, the `CONFIRMATION_REQUIRED` code carrying the delete/preserve lists with a re-issued call carrying `--yes` — never a prompt or a blocking read of stdin in that mode. A `<target>` matching no registered template's applied `targets` array **MUST** be refused with `TARGET_NOT_APPLIED`. A `<target>` that IS matched, but whose own on-disk location can be resolved and is no longer an ordinary directory — replaced by a regular file, or any other non-directory entry, since it was applied — **MUST** be refused with `CONTENT_CONFLICT`, naming the target, on every plan computation this flow makes (never only the first): the target's recorded content can no longer be enumerated, compared, or removed the way a directory's content is, the identical fact `apply` and `upgrade` already refuse for a payload path whose disk shape cannot be compared, and this refusal **MUST** reach the caller as the same one-envelope-on-stdout/non-zero-exit structured shape every other refusal in this feature already does — never an empty `--json` envelope or a bare, unstructured filesystem error. The system **MUST** support a non-destructive `--dry-run` that reports the identical lists without deleting anything and without requiring confirmation, and **MUST** recompute the deletion plan at confirmed-execution time rather than trusting an earlier computed plan. The owning template's declared `excludedSubtrees` **MUST** be resolved as the UNION of its project state entry's RECORDED value (`cpt-frontx-contract-project-provenance`'s `TemplateEntry.excludedSubtrees`) and its origin's CURRENT manifest declaration, whenever EITHER is established — never one overriding the other: two earlier resolution orders ("RECORDED always wins", then "CURRENT always wins") each mirrored the hazard the other one fixed, letting either a stale record widen a plan once its origin vanished, or a manifest narrowed after `apply` un-protect ground the recorded declaration already protected. The union computation **MUST NOT** default either source to `[]` for any fact short of an actually-established declaration — an entry carrying no RECORDED value and whose CURRENT manifest also cannot supply one (genuinely ABSENT, unreadable as a regular file, its origin folder no longer resolvable inside the project root, or failing the four-field contract) **MUST** refuse with `CONTENT_CONFLICT`, naming the template, rather than silently substituting an empty declaration for the missing side — a plan whose exclusions could not be verified states a blast radius nobody checked. The refusal's named remedy **MUST** depend on which failure this is: an origin confirmed genuinely absent names `unregister <name>` (`cpt-frontx-feature-composed-provenance`'s own orphan-drop escape, since nothing is left to re-register), never `register <origin> --replace`, which the same state proves cannot succeed; a merely unreadable origin still names `register <origin> --replace`. Whenever the two sources disagree, the computed plan **MUST** report that drift (both values) on its own returned shape, in every outcome it reaches (`--dry-run`, `CONFIRMATION_REQUIRED`, declined, and a completed deletion), so a developer confirming the deletion can see that they do (`target`).
...
- [x] For a target not yet recorded, existing on-disk content matching the payload exactly is reported as `identicalFiles` and causes no refusal; content differing from the payload at a payload path is reported as `contentConflicts` and refuses the whole batch, writing no file; content at a path the payload does not declare is reported as `additionalPaths` and refuses the batch unless `--adopt-existing` is given, in which case it is left untouched. A payload path standing on, or beneath, a symlink, beneath a regular file occupying a directory's position, standing where a DIRECTORY occupies the exact position the payload declares a file, or standing on, or beneath, a special filesystem entry (a FIFO, socket, or device), is always `contentConflicts` — never `identicalFiles`, even when a symlink's target happens to hold byte-identical content, and never adopted, since `--adopt-existing` means leaving what is already there untouched and a write through a symlink, into a directory a file already occupies, or into a directory position a file already occupies, touches something else entirely; a special filesystem entry is never even opened for reading, since doing so can block indefinitely; the refusal names the specific offending component and its kind.
...
- [x] `delete <target>` on a path that DOES match a registered template's applied `targets`, but whose own on-disk location is no longer an ordinary directory, is refused with `CONTENT_CONFLICT`, naming the target — as one envelope on stdout in `--json` and a message on stderr otherwise, with a non-zero exit — never a hang, an empty `--json` envelope, or an unstructured internal error, and never an apparently-empty deletion plan silently reporting nothing to delete or preserve.
- [x] `delete <target>` refuses with `CONTENT_CONFLICT` — never computing `toDelete`/`toPreserve` from an exclusion set silently emptied, and never placing a path either source would have declared excluded onto `toDelete` — only when NEITHER the owning template's CURRENT manifest (unreadable as a regular file — a FIFO, a socket, a device, a directory, a dangling symlink — genuinely absent, or failing the four-field contract) NOR its project state entry's RECORDED `excludedSubtrees` can establish a declaration; an entry carrying either one alone still succeeds, computed from that one source.
- [x] A deletion plan's declared `excludedSubtrees` is the UNION of the CURRENT manifest's declaration and the RECORDED one, each defaulting to empty only when the OTHER is established: narrowing the vendored manifest after `apply` never un-protects ground the RECORDED declaration protected when the template was applied, and a vanished origin (no CURRENT source at all) never widens the plan beyond what the RECORDED declaration allows. When the two sources disagree, the plan's own reported shape (`--dry-run`, `CONFIRMATION_REQUIRED`, declined, and a completed deletion alike) carries an `exclusionsDrift` fact naming both values; when they agree, or either one is unestablished, no such fact is reported.
- [x] `delete <target>`'s `CONTENT_CONFLICT` refusal for an owning template whose declared `excludedSubtrees` could not be established names the remedy that can actually run: `unregister <name>` when the origin is confirmed genuinely absent (nothing is left to re-register), `register <origin> --replace` when the origin is merely unreadable (present but not a regular file, which may still be repaired).
- [x] `unregister <name>` on a name whose `targets` array is non-empty still refuses with `TARGETS_EXIST` in the ordinary case — a resolvable origin, a recorded `excludedSubtrees`, or an origin that is merely temporarily unreadable rather than confirmed absent — but succeeds, dropping the entry and leaving every target's files untouched on disk, when the entry is an unusable orphan: its origin confirmed genuinely absent AND no `excludedSubtrees` ever recorded, so none of its targets could ever be reconciled into a deletion plan by any route (`cpt-frontx-algo-composed-provenance-unregister`).
- [x] `delete <target>` on a path that does not match any registered template's applied `targets` names, in its `TARGET_NOT_APPLIED` refusal, exactly the spelling the caller typed — never a canonicalized or otherwise resolved form the caller did not write.
