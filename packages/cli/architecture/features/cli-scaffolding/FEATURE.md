...
- The owning template's project state entry carries no RECORDED `excludedSubtrees` (a legacy entry from before that field existed) and its declaration cannot otherwise be established from its current manifest — the manifest exists but is not a regular file (a FIFO, a socket, a device, a directory, or a dangling symlink stands at its manifest path), or it is genuinely ABSENT, or its origin folder no longer resolves inside the project root, or it was read but no longer validates against the four-field contract: the CLI refuses with `CONTENT_CONFLICT` in every one of these cases, naming the template (and, for the unreadable case, the unreadable path), rather than computing `toDelete`/`toPreserve` from an exclusion set silently emptied by a fact it could not honestly answer — an origin folder removed from disk is the ORDINARY lifecycle of a vendored `path:` origin, not an exotic failure, and treating it as `[]` previously let a developer's own `excludedSubtrees`-protected file land in `toDelete` under `ok:true` (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-if-manifest-unreadable`/`inst-dp-if-declaration-unestablished`). An owning template WITH a recorded declaration never re-resolves the manifest at all (`inst-dp-if-recorded-exclusions`), so none of this applies to it.
...
4. [x] - `p1` - **IF** `<target>` is not found in any template's `targets` array - `inst-del-if-not-applied`
   ...
   3. [x] - `p1` - **ELSE IF** the deletion-plan algorithm refuses because the owning template's manifest cannot be read as a regular file (`cpt-frontx-algo-cli-scaffolding-delete-plan`'s own `inst-dp-if-manifest-unreadable`) — this flow does not re-derive that refusal a second way, it surfaces the algorithm's own `CONTENT_CONFLICT` unchanged - `inst-del-if-manifest-unreadable`
      1. [x] - `p1` - **RETURN** `CONTENT_CONFLICT`, naming the unreadable manifest path; nothing deleted, and `<target>` stays in its template's `targets[]` - `inst-del-return-manifest-unreadable`
...
3. [x] - `p1` - Compute `<target>`'s effective ownership by the same six-term subtraction the apply path computes (`inst-ua-compute-ownership`, CLI-5): `<target>` minus the owning template's declared `excludedSubtrees` (established by the sub-steps below) minus `projectOwnedRoots` beneath it minus `.frontx` minus the reserved environment entries (`.git`, `.DS_Store`, `Thumbs.db`) beneath it minus the template's own local origin folder (when installed by local path). The local-origin term is not optional here: a `path:`-installed template whose origin folder sits beneath its own target would otherwise have that folder — the developer's own source for the template — computed into this target's owned ground and removed - `inst-dp-compute-ownership`
   1. [x] - `p1` - **IF** the owning template's project-state entry carries a RECORDED `excludedSubtrees` (`cpt-frontx-contract-project-provenance`'s `TemplateEntry.excludedSubtrees` — set when the name is registered, and re-recorded from the NEW payload's own declaration at the name's most recent upgrade) — use it directly, never re-resolving the origin's current manifest. Deletion must never depend on that origin still being resolvable: a vendored `path:` origin folder is transient by design (`QUICK_START` §4 vendors it into the project, and a developer may remove it once applied), and re-resolving on every delete previously let that ordinary removal silently WIDEN `toDelete` to include the developer's own `excludedSubtrees`-protected files, under `ok:true` - `inst-dp-if-recorded-exclusions`
   2. [x] - `p1` - **ELSE** (no recorded declaration — a legacy entry from before this field existed) resolve the declared exclusions from the owning template's CURRENT manifest, refusing rather than substituting an empty declaration when one cannot honestly be established - `inst-dp-else-resolve-manifest`
      1. [x] - `p1` - **IF** the manifest exists but cannot be read as a regular file — a FIFO, a socket, a device, a directory, or a dangling symlink stands where it is expected - `inst-dp-if-manifest-unreadable`
         1. [x] - `p1` - **RETURN** `CONTENT_CONFLICT`, naming the unreadable manifest path; nothing deleted, and `<target>` stays in its template's `targets[]`. A real declaration exists at that path and this step simply could not read it — computing `toDelete`/`toPreserve` from an exclusion set silently emptied by a read failure would state a blast radius nobody verified, the same false-success class already closed for a FIFO or dangling symlink found INSIDE the target (`inst-dp-find-unenumerable`) applied here to a manifest this algorithm reads to decide the target's own exclusions - `inst-dp-return-manifest-unreadable`
      2. [x] - `p1` - **ELSE IF** the manifest could not establish a declaration at all — it is genuinely ABSENT (nothing exists at its recorded manifest path), its origin folder can no longer be proven to stay inside the project root (an escaping or dangling symlink), or it was read but no longer validates against the four-field contract - `inst-dp-if-declaration-unestablished`
         1. [x] - `p1` - **RETURN** `CONTENT_CONFLICT` naming the owning template and saying its declared `excludedSubtrees` could not be established, with the remedy: re-register the origin (`frontx register <origin> --replace`) so the declaration is recorded; nothing deleted. NEVER treated as `[]` — a live repro disproved that shape: an origin folder removed from disk (the ordinary lifecycle of a vendored `path:` origin, not an exotic failure) or an escaping symlink previously widened `toDelete` to include the developer's own `excludedSubtrees`-protected file under `ok:true`. The independent nested-target safety net (`inst-dp-find-nested`) is no substitute and does not justify folding this to `[]`: it protects a DIFFERENT template's own nested target, never the OWNING template's own declared exclusions - `inst-dp-return-declaration-unestablished`
