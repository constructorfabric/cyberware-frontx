...
1. [x] - `p1` - Look up the named entry in the inventory index - `inst-bupd-lookup`
   1. [x] - `p1` - **IF** the inventory index's own file cannot be parsed as JSON, or parses to something other than the tracked name-to-entry mapping every reader of it assumes (a hand-edited or corrupted `index.json`) - `inst-bupd-lookup-guard`
      1. [x] - `p1` - **RETURN** `CONTENT_CONFLICT` naming the index file; do not write to local inventory. This is a fact about content this local inventory store itself wrote — it is refused the same way any other unreadable content this feature already reports is, rather than reaching a caller as a raw parse error or a crash on a field the malformed shape does not carry - `inst-bupd-lookup-guard-fail`
