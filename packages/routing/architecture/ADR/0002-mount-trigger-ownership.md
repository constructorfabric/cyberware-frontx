...
Updated 2026-09-09: the cross-runtime-isolation driver and the Decision Outcome above now cross-reference `cpt-frontx-routing-adr-domain-occupancy-addressing-granularity` (ADR 0003, decided the same day), whose grammar scopes every domain's own resolution to the entries carrying its own domain key, narrowing that secondary driver structurally — a domain no longer depends on navigation's silence alone to avoid reacting to another domain's own entries. This does not change which option this record chose, its primary single-writer-occupancy-authority rationale, or its outcome; quoted fragments and "runtime segment" phrasing were also aligned to current wording ("report its own diff" / "reported diff"; ADR 0003's own "domain"/"entries" vocabulary).
