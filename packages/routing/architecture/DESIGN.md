...
| Extension token | The `name`-alphabet value an entry's own extension segment carries — the extension registration's own normalized route-identity value, a field the `mfes` package's own contract declares (`cpt-frontx-routing-adr-occupant-identity-stability`), uninterpreted but required to be lexically well-formed because it is a literal substring of an entry this package composes and parses (`cpt-frontx-routing-adr-domain-occupancy-addressing-granularity`; §2.3, O3a). | `name`-alphabet string |
