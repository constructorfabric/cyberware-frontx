...
| extension token | The `name`-alphabet value an entry's own extension segment carries: the value the extension registration itself declares as its own routable identity, normalized by stripping one leading `/`. Which concrete field on a registration supplies that value is the `mfes` package's own contract to declare, not a schema this glossary defines. A registration whose normalized value is not a valid `name`, or that supplies none at all, is not routable and is never projected. |
